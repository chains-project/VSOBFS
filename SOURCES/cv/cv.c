...
void cvlong(i32_t *l);
void cvshort(i16_t *s);
void writef(char *addr, int sz, i32_t cnt);
int is_rest_local(struct outname *A, int i);
void put(i32_t l, int sz);
#endif
	...
	char	*argv[];
{
	...
	text_off= entry & ~(i32_t)(PAGE_SIZE-1);
	uzp= text_off > 0;
	pal= entry > text_off;

	/* A few checks */
#if 0 /* this check is invalid if e.g. sizeof(i32_t) != 4 -- an */
	...
	{	/* separate I&D */
		...
		outsect[TEXTSG].os_size = mnx_h.a_text=
			align(outsect[TEXTSG].os_size, (i32_t)HCLICK_SIZE);
	}
	else
	{
		outsect[TEXTSG].os_size = mnx_h.a_text =
	...
	/* Action at last */
	lseek(output,(i32_t) (off_t)sizeof(mnx_h),0);
	...
	cvlong(&(mnx_h.a_syms));
	writef((char *)&mnx_h, 1, (i32_t) sizeof(mnx_h));
	...
	register struct machrelo *mp;
{
	...
	i32_t correction = 0;
	i32_t where = TOT_HDRSIZE+ap->or_addr;
	i32_t X;
	i32_t here;
	int sz;
	...
	here = lseek(output, (off_t)0, 1);
	lseek(output, (off_t)where, 0);
	sz = ap->or_type & RELSZ;
	X = get(sz) + correction;
	lseek(output, (off_t)where, 0);
	put(X,sz);
	lseek(output, (off_t)here, 0);
}
#endif
...
pcix_emit_symtab()
{
	...
	i32_t l;
	i32_t off = OFF_CHAR(outhead);
	int j;
	char *p, *chars;

	if (outhead.oh_nname == 0) return;	/* no symbol table to emit */

	if (((u32_t) outhead.oh_nchar != outhead.oh_nchar) ||
	     (outhead.oh_nchar != 0 &&
	      (chars = malloc((size_t)outhead.oh_nchar)) == 0)) {
		fatal("No memory.\n");
	}
	...
	for (i = 0; i < outhead.oh_nname; i++) {
		...
		cvlong((i32_t *) &IX_name.n_value);
		cvshort((i16_t *) &IX_name.n_type);
		writef((char *) &IX_name, 1, (i32_t) sizeof(IX_name));
	}
}
...
bsd_emit_symtab()
{
	register u16_t i = outhead.oh_nname;
	register struct outname *A;
	struct sym *MACHnames;
	register struct sym *M;
	char *chars;
	i32_t offX = OFF_CHAR(outhead) - 4;
	...
	writef((char *)MACHnames, sizeof(struct sym), (i32_t) outhead.oh_nname);
	free(MACHnames);
	free(ACKnames);
	if ((u32_t) outhead.oh_nchar != outhead.oh_nchar ||
	    !( chars = malloc((off_t) outhead.oh_nchar))) {
		fatal("No memory\n.");
	}
