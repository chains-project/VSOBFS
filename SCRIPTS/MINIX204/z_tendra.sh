...
make_mydata()(
  [ -d MYDATA_TMP ] && find MYDATA_TMP -type d | xargs chmod u+w && rm -rf MYDATA_TMP
  mkdir MYDATA_TMP
  cd MYDATA_TMP
  sed '
/@---LASTBL---@/s||'"$lastbl"'|g
...
# without 'o' tar (at least Minix-2 tar) "restores" totally irrelevant uid/gid data
  ...
  ( set -x
    rm rc
    sh /mtop/z__tendra.sh || >/Trouble ) 2>&1 | tee tendraLog00
  [ ! -f /Trouble ]
...
____
  ...
  cp "$S"/z__tendra.sh \
     "$A"/SOURCES/SYS.TAZ \
     mtop
  gzip -d <"$A"/SOURCES/TenDRA412b.tgz | compress >mtop/TenDRA412b.TAZ
...
(
  [ -d "$tdir" ] && find "$tdir" -type d | xargs chmod u+w && rm -rf "$tdir" && rm -rf "$tdir"
  mkdir "$tdir"
  cd "$tdir"
