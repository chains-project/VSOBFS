...
# _here_and_now_
###CC='hostcc -Dregister= -Drindex=strrchr -DS_IFMT=0170000 -DS_IFREG=0100000 -DS_IFDIR=0040000'
CC='hostcc -std=c89 -D_PROTOTYPE\(x,y\)=x\ y -Dregister= -Drindex=strrchr -DS_IFMT=0170000 -DS_IFREG=0100000 -DS_IFDIR=0040000'
export CC
...
myCC(){
###  ( set -x; hostcc -Dregister= -Drindex=strrchr -DS_IFMT=0170000 -DS_IFREG=0100000 -DS_IFDIR=0040000 "$@" )
  ( set -x; hostcc -std=c89 -D_PROTOTYPE\(x,y\)=x\ y -Dregister= -Drindex=strrchr -DS_IFMT=0170000 -DS_IFREG=0100000 -DS_IFDIR=0040000 "$@" )
}
mynewerCC(){
###  ( set -x; hostcc -Dregister= -Drindex=strrchr -DS_IFMT=0170000 -DS_IFREG=0100000 -DS_IFDIR=0040000 "$@" )
  ( set -x; hostcc -D_PROTOTYPE\(x,y\)=x\ y -Dregister= -Drindex=strrchr -DS_IFMT=0170000 -DS_IFREG=0100000 -DS_IFDIR=0040000 "$@" )
}
...
mynewerCC -s -o "$top"/bin/sltar "$A"/SOURCES/sltar.c
#
# our reproducible-tweaked mkproto
# (set PATH_MAX to allow more advanced contents in a file system)
mynewerCC -s -o "$top"/bin/mkproto -DPATH_MAX=4096 "$A"/SOURCES/mkproto.c
###
...
mybareCC(){
  ( set -x
    hostcc -std=c89 \
           ...
           -Dregister= -Drindex=strrchr -DS_IFMT=0170000 -DS_IFREG=0100000 -DS_IFDIR=0040000 \
           \
           "$@"
  )
}
...
/int rootdir/s|int|void|
/int eat_dir/s|int|void|
/int eat_file/s|int|void|
/int enter_dir/s|int|void|
/int add_zone/s|int|void|
/int get_block/s|int|void|
/int flush/s|int|void|
/int cache_init/s|int|void|
/int local_getline/s|int|void|
/^eat_dir/s|^|void |
/^eat_file/s|^|void |
/^enter_dir/s|^|void |
/^add_zone/s|^|void |
...
/^cache_init/s|^|void |
/^rootdir/s|^|void |
/^local_getline/s|^|void |
/mode="/s|="|=%06o"|
/printf("%6ld", inode\[i]\.d2_size);/s|ld|d|
/"  zone\[0]=%ld/s|ld|d|
'
