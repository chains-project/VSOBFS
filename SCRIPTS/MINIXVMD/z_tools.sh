...
# generally, we avoid "ln -s", for the sake of possible host platforms
# which lack symlinks; a copy can also be problematic, if a binary/script
# only works when run from a certain location (e.g. looks for its
# resources there) -- you have been warned
  ln -s "$localtool" top/bin/"$tool"
###  cp "$localtool" top/bin/"$tool"
done
