...
case "$sysimage" in
(+*)
# image was too big for mkfs, put its data outside the file system
  sysimage="$(expr "$sysimage" : '.\(.*\)')"
# add the boot monitor code and the bootable image
# to the file system (just past its end)
  installboot -d root_fs "$bootblock" "$bootprog" "$sysimage"
  ;;
(*)
# this uses the boot program and system image data inside the file system:
