...
makes_no_size_difference(){
### nevertheless, the size optimization is needed
### for various components, to make em_cemcom.ansi smaller
CC="cc -OS"

#### nevertheless, we want to apply size optimization
#### to em_cemcom.ansi, which otherwise becomes too big
#### to put into file system via mkfs (the limit is 256KB)
CC_FOR_EM_CEMCOM="cc -OS"
fix make/Make.cemcom '
/\$(CC)/s||'"$CC_FOR_EM_CEMCOM"'|
'
}
# unfortunately, the em_cemcom.ansi file has to be tranfered
# to the target file system in a special way,
# because of the mkfs limitation
# END makes_no_size_difference()

# as for specifically building ego/cf itself:
# we embed the values directly into the relevant Makefile
# without putting them in the environment:
CC_FOR_EGO_CF="cc -O"
...
# adjust for Minix-vmd path policies
  mv lib/ego/* lib/ack/ego
  rmdir lib/ego
) || exit
...
# -------- we have already the source of cv at hand
#          but it is to be rebuild as a native binary
( set -x
  ( cd "$B"/cv
    cc -s -o "$mtop"/lib/ack/cv cv.c rd.c rd_bytes.c
  )
) || exit

# ---------- provide crtfiles and libraries

cp -v "$top"/lib/ack/i386/*.o "$mtop"/lib/ack/i386
cp -v "$top"/lib/ack/i386/*.a "$mtop"/lib/ack/i386
#####cp -v "$top"/lib/ack/i86/*.o  "$mtop"/lib/ack/i86
cp -v "$top"/lib/ack/i86/*.a  "$mtop"/lib/ack/i86

# ---------- make cc usable, by providing a descr for acd

# reuse the earlier read unfixed descr
cp "$top"/lib/ack/descr_ "$mtop"/lib/ack/descr

# NOTE here we anticipate the absolute paths
# to be used when running natively
fix "$mtop"/lib/ack/descr '
/^L =/s|/.*|/mtop/lib|
/\/usr/s||/mtop|g
/AAL cr/s||AAL Dcr|
/^ACK_CPP =/s|$| -I/mtop/include|
/^ACK_CEM =/s|CPP_F|& -I/mtop/include|
'
# keep "$mtop"/lib/ack/descr_ for further later reuse
rm "$mtop"/lib/ack/descr.
