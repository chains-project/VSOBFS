...
# under Minix-vmd
# use Tiny C Compiler
# to build Tiny C Compiler using Minix-vmd libraries
#
# 4 arguments:
...
____
  mkdir mtop
# here we use an addtional script
  cp "$S"/z__"$tag".sh "$S"/z__tivlib.sh mtop
# we will need Minix-vmd library sources
  gzip -d <"$A"/SOURCES/MINIX-VMD.SRC.TGZ |
                             compress > mtop/SRC.TAZ
